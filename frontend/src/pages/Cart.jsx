...
const startHold = (id, quantity, type) => {
  holdActiveRef.current = false;

  let currentQty = qtyRef.current[id] ?? quantity;

  // wait 300ms → detect HOLD
...
const stopHold = async (id, quantity, type) => {
  clearTimeout(holdTimeoutRef.current);
  clearInterval(holdIntervalRef.current);
  ...
  // ⭐ HOLD RELEASE → ONE API CALL
  const finalQty = qtyRef.current[id];

  if (finalQty !== undefined) {
    await handleQuantityChange(id, finalQty);
  }

  delete qtyRef.current[id];
    ...
    <div className="px-4 sm:px-8 py-6 text-black dark:text-white min-h-screen">
      ...
      <div className="max-w-6xl mx-auto w-full">
        <h2 className="text-3xl font-extrabold mb-8 text-center bg-clip-text text-transparent bg-gradient-to-r from-[#FF5722] to-[#FFD54F] flex items-center justify-center gap-2">
          <Lottie
            animationData={shoppingCartAnim}
            className="h-12 w-12"
            loop
            autoplay
          />
        ...
        <AnimatePresence>
          {cartItems.length === 0 ? (
            ...
            >
              ...
              />
              <p className="text-xl font-semibold text-[#FF5722] mb-4">
                Your cart is empty!
              </p>
              <Link
                to="/menu"
  ...
  whileTap={{ scale: 0.9 }}
  onPointerDown={() =>
    startHold(item.foodId._id, item.quantity, "dec")
  }
  onPointerUp={() =>
    stopHold(item.foodId._id, item.quantity, "dec")
  }
  onPointerCancel={() =>
    stopHold(item.foodId._id, item.quantity, "dec")
  }
...
>
  <Minus size={16} />
                        ...
                        <span className="px-3 text-lg font-semibold">
                          {localQty[item.foodId._id] ?? item.quantity}
                        </span>
                        <motion.button
  whileTap={{ scale: 0.9 }}
  onPointerDown={() =>
    startHold(item.foodId._id, item.quantity, "inc")
  }
  onPointerUp={() =>
    stopHold(item.foodId._id, item.quantity, "inc")
  }
  onPointerCancel={() =>
    stopHold(item.foodId._id, item.quantity, "inc")
  }
              ...
              <div className="w-full lg:w-1/3 lg:sticky lg:top-8 h-fit bg-white/80 dark:bg-[#1E1E1E] backdrop-blur-lg border border-gray-200 dark:border-[#2A2A2A] rounded-2xl p-6 shadow-lg space-y-6">
                <div className="text-center lg:text-left space-y-1">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
                    <Truck className="w-4 h-4 text-[#FFD54F]" />
