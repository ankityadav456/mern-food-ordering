...
const Navbar = ({ isCollapsed, toggleSidebar, isSidebarVisible, isDrawerMode, isZoomed }) => {
  ...
  const { user, logout } = useAuth(); // Assuming cartItems are in AuthContext  
  const { cartItems } = useCart(); // Assuming cartItems are in AuthContext
  ...
  return (
    <nav
      className={`fixed top-0 right-0 z-50 h-18 bg-[#0d0d0d] border-b border-[#2A2A2A] transition-all duration-300
        ...
        <div className="flex items-center space-x-4 ml-auto relative z-50">
          <Link to="/cart" className="relative group">
            ...
            {cartItems && cartItems.length > 0 && (
              <div className="absolute top-0 right-0 bg-[#D4AF37] text-black text-xs font-semibold rounded-full w-5 h-5 flex items-center justify-center">
                {cartItems.length}
              </div>
            )}
          </Link>
